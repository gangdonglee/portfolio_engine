...
namespace engine::render
{
    ...
    CommandQueue::CommandQueue(Device& device)
    {
        ...
        ThrowIfFailed(
            d3dDevice->CreateCommandQueue(
                &desc,
                IID_PPV_ARGS(_queue.ReleaseAndGetAddressOf())),
    ...
    CommandQueue::~CommandQueue()
    {
        ...
        if (_queue && _fence && _fenceEvent != nullptr)
        {
            try
            {
                FlushGpu();
    ...
    {
        // 실패 시 _fenceValue 가 비대칭 증가하지 않도록, 로컬 next 로 먼저 시그널하고
        // 성공 후에만 멤버를 갱신한다.
        const std::uint64_t next = _fenceValue + 1;
        ThrowIfFailed(
            _queue->Signal(_fence.Get(), next),
    ...
    void CommandQueue::WaitForFenceValue(std::uint64_t value)
    {
        ...
        ThrowIfFailed(
            _fence->SetEventOnCompletion(value, static_cast<HANDLE>(_fenceEvent)),
        ...
        const DWORD waitResult = ::WaitForSingleObject(
            static_cast<HANDLE>(_fenceEvent),
            kWaitTimeoutMs);
